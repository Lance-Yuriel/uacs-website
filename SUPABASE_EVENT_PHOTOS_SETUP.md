...
After creating the bucket, you need to set up policies for secure access. Only authenticated admins can upload/update/delete photos, but everyone can view them.
...
#### Policy 2: Allow Authenticated Uploads (Admin Only)
...
#### Policy 3: Allow Authenticated Updates (Admin Only)
1. Click **New Policy** again
2. Choose **For full customization**, click **Next**
3. Configure:
...
#### Policy 4: Allow Authenticated Deletes (Admin Only)
1. Click **New Policy** again
2. Choose **For full customization**, click **Next**
3. Configure:
...
1. Try uploading an image through the admin event form
...
event-photos/
  └── events/
      ├── event-001-1234567890.jpg
      ├── event-002-1234567891.png
      └── ...
```
...
- **Solution**: Make sure the bucket name is exactly `event-photos` (case-sensitive, lowercase with hyphen)

**Issue**: "Permission denied" error
- **Solution**: Check that RLS policies are correctly set up and that the user is authenticated (logged in as admin)
